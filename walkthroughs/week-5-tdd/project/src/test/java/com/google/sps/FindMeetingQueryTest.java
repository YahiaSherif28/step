...
public final class FindMeetingQueryTest {
  ...
  @Test
  public void optionalWithWholeDayEvent() {
    // Have each person have different events. We should see two options because each person has
    // split the restricted times.
    //
    //           |-----------C-----------------| C is optional
  ...
  @Test
  public void optionalWithSmallEvent() {
    // Have each person have different events. We should see two options because each person has
    // split the restricted times.
    //
    //                       |-C-| C is optional
  ...
  public void justEnoughRoomWithOptional() {
    // Have one person and one optional person, but make it so that there is just enough room at one
    // point
    // in the day to have the meeting. Optional person is ignored as the slot would have been too
    // small.
    //                 |-B-|       B is optional
    // Events  : |--A--|     |----A----|
    // Day     : |---------------------|
    // Options :       |-----|     B is ignored
  ...
  public void optionalOnlyWithGaps() {
    ...
    Collection<Event> events =
        Arrays.asList(
            ...
            new Event(
                "Event 3",
                TimeRange.fromStartDuration(TIME_1100AM, DURATION_30_MINUTES),
                Arrays.asList(PERSON_A)));
  ...
  @Test
  public void optionalOnlyWithNoGaps() {
    // No mandatory attendees, just two optional attendees with no gaps in their schedules.
    // query should return that the whole day is available since the optional attendees
    // cannot be accommodated and the mandatory ones (all zero of them) are free all day.
    //
    // Events  : |-------A-------||-----B------||-----A-----|  All are optional
    // Day :     |------------------------------------------|
    // Options   |------------------------------------------| Ignore all optional
    Collection<Event> events =
        Arrays.asList(
            new Event(
                "Event 1",
